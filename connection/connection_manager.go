...
type BroadcastChannel struct{
	Channel chan notification.Packet
	Active bool
...
var subscriptionPassword string
...
func WebsocketConnectionHandler(ws *websocket.Conn) {
  ...
  authorized := false
  var DesiredMetaData notification.MetaData
...
func InitializeConnectionManager(SubscriptionPassword string, DataStream chan notification.Packet){

	subscriptionPassword = SubscriptionPassword

	defaultFeeds := []int{1,2}
	defaultSources := notification.Sources{Corporate:true,SocialMedia:true,Aggregate:true}

	DefaultSubscription := notification.MetaData{defaultFeeds,defaultSources}
	...
	for{
		...
		select{
			...
			case <- BroadcastSecretary:
				...
				newChannel := BroadcastChannel{make(chan notification.Packet),false,DefaultSubscription}
